...
@dataclass
class VSR_PTDConfig(AVHubertSeq2SeqConfig):
    su2sw_embedding: int = field(default=1002)
    su2sw_embedding_dim: int = field(default=768)
    su2sw_cross_att_head: int = field(default=1)
    su2sw_pth: str = field(default='/path/to/su2sw')
    
@register_model("VSR_PTD", dataclass=VSR_PTDConfig)
class VSR_PTD(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, cfg, task):
        ...
        if cfg.w2v_args is None:
            ...
            print(f"state loaded from {avhubert_pth}:")
            print(state)
        ...
        checkpoint_path = cfg.su2sw_pth
        # 加载整个对象，通常参数在 'model' 键下
        checkpoint = torch.load(checkpoint_path)
        print(f"Checkpoint loaded from {checkpoint_path}:")
        print(checkpoint)
        su2sw_state_dict = checkpoint['model']
        ...
        print("Loading audio_encoder...")
        encoder_dict = {}
        for k, v in su2sw_state_dict.items():
            if k.startswith('encoder.'):
                # 去掉前缀 "encoder." (长度为8)
                new_key = k[8:] 
                encoder_dict[new_key] = v
        
        missing_keys, unexpected_keys = audio_encoder.load_state_dict(encoder_dict, strict=True)
        print(f"Encoder load result - Missing: {len(missing_keys)}, Unexpected: {len(unexpected_keys)}")
        ...
        # 注意：TransformerDecoder 内部包含 embed_tokens。
        # Checkpoint 中有 'decoder.embed_tokens.weight'，去掉前缀后变为 'embed_tokens.weight'。
        # 这会自动加载到 decoder.embed_tokens 中（前提是 decoder 初始化时传入的 embedding 层名称对齐）。
        missing_keys, unexpected_keys = decoder.load_state_dict(decoder_dict, strict=True)
        print(f"Decoder load result - Missing: {len(missing_keys)}, Unexpected: {len(unexpected_keys)}")

        model = cls(encoder, decoder, tgt_dict, cfg, audio_encoder)


        print("Loading embedding...")
        if 'embedding.weight' in su2sw_state_dict:
            model.embedding.weight.data.copy_(su2sw_state_dict['embedding.weight'])
        else:
            print("Warning: 'embedding.weight' not found in checkpoint.")
            raise ValueError("Required weights not found in checkpoint.")

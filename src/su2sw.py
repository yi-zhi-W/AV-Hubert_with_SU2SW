...
@dataclass
class SU2SWConfig(AVHubertSeq2SeqConfig):
    su2sw_embedding: int = field(default=1002)
    su2sw_embedding_dim: int = field(default=768)
    
    
@register_model("su2sw", dataclass=SU2SWConfig)
class SU2SW(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder, tgt_dict, cfg):
        super().__init__(encoder, decoder)
        self.cfg = cfg
        self.freeze_finetune_updates = cfg.freeze_finetune_updates
        self.embedding = nn.Embedding(num_embeddings=cfg.su2sw_embedding, embedding_dim=cfg.su2sw_embedding_dim)
    ...
    def build_model(cls, cfg, task):
        ...
        transformer_enc_cfg.encoder_embed_dim = cfg.su2sw_embedding_dim
        print(f"Building model with encoder layers {transformer_enc_cfg.encoder_layers} and embedding dim {transformer_enc_cfg.encoder_embed_dim}")
    ...
    def forward(self, **kwargs):
        ...
        # print(f"Forward called with batch size {B} and token length {T_token}")

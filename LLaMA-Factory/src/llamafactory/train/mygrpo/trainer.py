...
class CustomGRPOTrainer(Trainer):
    ...
    def _generate_single_turn(self, inputs: list):
        ...
        if False and self.use_vllm:
            ...
            # 获取当前GPU的显存占用情况 (转换为 GB)
            allocated_mem = torch.cuda.memory_allocated(device) / (1024 ** 3)
            reserved_mem = torch.cuda.memory_reserved(device) / (1024 ** 3)
            
            # 获取当前 Batch 的形状 (Batch_size, Seq_length)
            input_shape = generate_inputs["input_ids"].shape
            
            # 强制刷新输出流，防止多卡打印日志混乱
            print(f"\n[GPU {process_id} 标识] 即将进入 generate! "
                  f"当前输入形状: {input_shape}, "
                  f"已分配显存: {allocated_mem:.2f} GB, "
                  f"保留显存: {reserved_mem:.2f} GB\n", flush=True)
            ...
            with (unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model, torch.no_grad(), nullcontext()):
                ...
                # Unmerge adapters while parameters are still gathered
                    ...
                    outputs = self.llm.generate(generate_inputs, sampling_params=self.sampling_params, use_tqdm=False)

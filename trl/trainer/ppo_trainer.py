...
class PPOTrainer(BaseTrainer):
    ...
    def step(
        ...
        # print(scores)
        if not (isinstance(scores, list) and scores[0].ndim >= 1 and scores[0].numel() > 1):
            scores = torch.tensor(scores, device=self.current_device)
            print("self.config.use_score_scaling", self.config.use_score_scaling)
            if self.config.use_score_scaling:
                ...
                if self.config.use_score_norm:
                    print("self.config.use_score_norm", self.config.use_score_norm)
            ...
            for backward_batch_start in range(0, bs, self.config.backward_batch_size):
                ...
                for mini_batch_start in range(0, self.config.backward_batch_size, self.config.mini_batch_size):
                    ...
                    with self.accelerator.accumulate(self.model):
                        ...
                        # print("mini_batch_dict", mini_batch_dict)
    ...
    def compute_rewards(
        ...
        for score, logprob, ref_logprob, mask in zip(scores, logprobs, ref_logprobs, masks):
            ...
            if score.ndim >= 1 and score.numel() > 1:
                # print("score.ndim == 2")
                ...
                # score是无padding的序列奖励，长度应等于有效token数
                valid_length = last_valid - first_valid + 1
                # print("valid_length", last_valid, first_valid)
                score_len = score.shape[0]
                score_tensor = score.to(reward.device)
                if score_len == valid_length:
                    # 正常情况：直接对齐
                    reward[first_valid: last_valid + 1] += score_tensor

                elif score_len < valid_length:
                    # score更短：右对齐到末尾（丢弃的多半是开头的padding/特殊token影响）
                    offset = valid_length - score_len
                    reward[first_valid + offset: last_valid + 1] += score_tensor

                else:
                    # score更长：截取与valid_length等长的末尾部分
                    reward[first_valid: last_valid + 1] += score_tensor[-valid_length:]
            else:
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        if isinstance(data["scores"],list):
            score_means = torch.stack([s.mean() for s in data["scores"]])  # 每条序列取均值 → shape (batch_size,)
            mean_scores = score_means.mean()
            std_scores = score_means.std()
        else:

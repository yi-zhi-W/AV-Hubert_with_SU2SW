...
def _insert_adapter_name_into_state_dict_for_otherlora(
    state_dict: dict[str, torch.Tensor], adapter_name: str, parameter_prefix: str
) -> dict[str, torch.Tensor]:
    # print("_insert_adapter_name_into_state_dict_for_mtllora")
    def transform_string(input_str):
        pattern = r'(lora_[a-zA-Z]+)_(\d+)(\.default)(\.weight)'
        replacement = r'\1\3_\2\4'
...
def _insert_adapter_name_into_state_dict_for_mtllora(
    state_dict: dict[str, torch.Tensor], adapter_name: str, parameter_prefix: str
) -> dict[str, torch.Tensor]:
    # print("_insert_adapter_name_into_state_dict_for_mtllora")
    def transform_string(input_str):
        pattern = r'(mtllora_[a-zA-Z]+)_(\d+)(\.default)(\.weight)'
        replacement = r'\1\3_\2\4'
        return re.sub(pattern, replacement, input_str)
...
def set_peft_model_state_dict(
    ...
    for name, module in model.named_modules():
        if isinstance(module, AuxiliaryTrainingWrapper):
            ...
            print("isinstance(module, AuxiliaryTrainingWrapper)")
    ...
    if config.is_prompt_learning or config.peft_type == PeftType.ADAPTION_PROMPT:
        ...
        if config.peft_type == PeftType.MTLLORA or config.peft_type == PeftType.BASEMTLLORA or config.peft_type == PeftType.COMBINEDMTLLORA or config.peft_type == PeftType.GATEMTLLORA or config.peft_type == PeftType.COMSCALMTLLORA or config.peft_type == PeftType.COMBROUTEMTLLORA:
            peft_model_state_dict = _insert_adapter_name_into_state_dict_for_mtllora(
                state_dict, adapter_name=adapter_name, parameter_prefix=parameter_prefix
            )
        elif config.peft_type == PeftType.MULTILORA:
            peft_model_state_dict = _insert_adapter_name_into_state_dict_for_otherlora(
                state_dict, adapter_name=adapter_name, parameter_prefix=parameter_prefix
            )
        else:

...
from .layer import BaseMTLLoraConv2d, BaseMTLLoraLayer, dispatch_BaseMTLLora_lora
...
class BaseMTLLoraModel(BaseTuner):

    prefix: str = "basemtllora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

        print("*********************BaseMTLLoraModel __init__****************************")

    def _check_new_adapter_config(self, config: BaseMTLLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: BaseMTLLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, BaseMTLLoraLayer) and not isinstance(target, AdaLoraLayer):
            raise NotImplementedError(f"isinstance(target, MTLLoraLayer) and not isinstance(target, AdaMTLLoraLayer).")
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, BaseMTLLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def _create_new_module(mtllora_config, adapter_name, target, **kwargs):
        ...
        dispatchers.extend(
            [
                dispatch_BaseMTLLora_lora
            ]
        )

        new_module = None
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, mtllora_config=mtllora_config, **kwargs)
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, BaseMTLLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, BaseMTLLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, BaseMTLLoraLayer) or isinstance(module, AuxiliaryTrainingWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, BaseMTLLoraLayer):
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        conv2d = isinstance(target, BaseMTLLoraConv2d)
        if conv2d:

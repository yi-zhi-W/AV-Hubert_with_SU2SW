...
from .config import BaseMTLLoraConfig
from .dora import DoraConv2dLayer, DoraConv3dLayer, DoraEmbeddingLayer, DoraLinearLayer, _DoraConvNdLayer
from peft.tuners._buffer_dict import BufferDict

class BaseMTLLoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "basemtllora_A", "basemtllora_B", "basemtllora_A_shared", "basemtllora_B_shared", "lora_embedding_A", "lora_embedding_B")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.basemtllora_A = nn.ModuleDict({})
        self.basemtllora_B = nn.ModuleDict({})
        self.basemtllora_A_shared = nn.ModuleDict({})
        self.basemtllora_B_shared = nn.ModuleDict({})
        # For Embedding layer
    ...
    def update_layer(
        ...
        raise NotImplementedError(f"BaseMTLLoraLayer can not update_layer")

    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        raise NotImplementedError(f"BaseMTLLoraLayer can not reset_lora_parameters")
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.basemtllora_A.keys():
                continue
...
class BaseMTLLoraLinear(nn.Module, BaseMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        BaseMTLLoraLayer.__init__(self, base_layer, **kwargs)
    ...
    def update_layer(
        ...
        self.basemtllora_A_shared[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)

        self.basemtllora_B_shared[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)

        self.task_num[adapter_name] = task_num
        for num in range(task_num):

            mtllora_name = adapter_name+"_"+str(num)
            self.basemtllora_A[mtllora_name] = nn.Linear(self.in_features, r, bias=lora_bias)

            self.basemtllora_B[mtllora_name] = nn.Linear(r, self.out_features, bias=lora_bias)
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            nn.init.zeros_(self.basemtllora_B[adapter_name].weight)
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, task_num, device: Optional[torch.device] = None) -> None:
        ...
        for num in range(task_num):
            adapter_layer = getattr(self, "basemtllora_A", None)
            basemtllora_A_name = adapter_name+"_"+str(num)
            if basemtllora_A_name not in adapter_layer:
                raise ValueError("basemtllora_A_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[basemtllora_A_name] = adapter_layer[basemtllora_A_name].to(device, dtype=dtype)
            else:
                adapter_layer[basemtllora_A_name] = adapter_layer[basemtllora_A_name].to(device)


        for num in range(task_num):
            adapter_layer = getattr(self, "basemtllora_B", None)
            basemtllora_B_name = adapter_name+"_"+str(num)
            if basemtllora_B_name not in adapter_layer:
                raise ValueError("basemtllora_B_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[basemtllora_B_name] = adapter_layer[basemtllora_B_name].to(device, dtype=dtype)
            else:
                adapter_layer[basemtllora_B_name] = adapter_layer[basemtllora_B_name].to(device)
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        for adapter_name in adapter_names:
            self.basemtllora_A_shared[adapter_name].requires_grad_(True)
            self.basemtllora_B_shared[adapter_name].requires_grad_(True)
            for num in range(self.task_num[adapter_name]):
                mtllora_name = adapter_name+"_"+str(num)
                self.basemtllora_A[mtllora_name].requires_grad_(True)
                self.basemtllora_B[mtllora_name].requires_grad_(True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, task_num):
        ...
        if adapter_name in self.basemtllora_A_shared.keys():
            if init_lora_weights is True:
                # initialize A the same way as the default for nn.Linear and B to zero
                # https://github.com/microsoft/LoRA/blob/a0a92e0f26c067cf94747bdbf1ce73793fa44d19/loralib/layers.py#L124
                nn.init.kaiming_uniform_(self.basemtllora_A_shared[adapter_name].weight, a=math.sqrt(5))
                for num in range(task_num):
                    nn.init.kaiming_uniform_(self.basemtllora_A[adapter_name+"_"+str(num)].weight, a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.basemtllora_A_shared[adapter_name].weight, std=1 / self.r[adapter_name])
                for num in range(task_num):
                    nn.init.normal_(self.basemtllora_A[adapter_name+"_"+str(num)].weight, std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_lora_weights=}")
            
            nn.init.zeros_(self.basemtllora_B_shared[adapter_name].weight)
            for num in range(task_num):
                nn.init.zeros_(self.basemtllora_B[adapter_name+"_"+str(num)].weight)

            if self.lora_bias[adapter_name]:
                nn.init.zeros_(self.basemtllora_B[adapter_name].bias)
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            find = False
            basemtllora_A_keys = self.basemtllora_A_shared.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in basemtllora_A_keys:
                    continue
                find = True

                basemtllora_A_shared = self.basemtllora_A_shared[active_adapter]
                basemtllora_B_shared = self.basemtllora_B_shared[active_adapter]

                basemtllora_A_task = self.basemtllora_A[active_adapter+"_"+str(language_id.item())]
                basemtllora_B_task = self.basemtllora_B[active_adapter+"_"+str(language_id.item())]

                # print("basemtllora_A_shared")
                # print(basemtllora_A_shared.weight)
                # print("basemtllora_B_shared")
                # print(basemtllora_B_shared.weight)
                # print("basemtllora_A_task")
                # print(basemtllora_A_task.weight)
                # print("basemtllora_B_task")
                # print(basemtllora_B_task.weight)

                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                x = self._cast_input_dtype(x, basemtllora_A_shared.weight.dtype)

                if not self.use_dora[active_adapter]:
                    after_shared = basemtllora_B_shared(basemtllora_A_shared(dropout(x))) * scaling
                    after_task = basemtllora_B_task(basemtllora_A_task(dropout(x))) * scaling

                    result = result + after_shared + after_task
                else:
            ...
            if not find:
                raise ValueError("active_adapter not in basemtllora_A_keys")
    ...
    def __repr__(self) -> str:
        rep = super().__repr__()
        return "basemtllora_." + rep


class BaseMTLLoraEmbedding(nn.Module, BaseMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        BaseMTLLoraLayer.__init__(self, base_layer)
...
class _BaseMTLLoraConvNd(nn.Module, BaseMTLLoraLayer):
...
class BaseMTLLoraConv2d(_BaseMTLLoraConvNd):
...
class BaseMTLLoraConv1d(_BaseMTLLoraConvNd):
...
class BaseMTLLoraConv3d(_BaseMTLLoraConvNd):
...
class BaseMTLLoraMultiheadAttention(nn.Module, BaseMTLLoraLayer):
...
def dispatch_BaseMTLLora_lora(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        new_module = BaseMTLLoraEmbedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = BaseMTLLoraConv2d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv3d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = BaseMTLLoraConv3d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, nn.Conv1d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = BaseMTLLoraConv1d(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = BaseMTLLoraMultiheadAttention(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = BaseMTLLoraLinear(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = BaseMTLLoraLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)

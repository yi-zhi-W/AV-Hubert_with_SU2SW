...
from .layer import MULTILoraConv2d, MULTILoraLayer, dispatch_MULTILora_lora
...
class MULTILoraModel(BaseTuner):

    prefix: str = "multilora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

        print("*********************MULTILoraModel __init__****************************")

    def _check_new_adapter_config(self, config: MULTILoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: MULTILoraConfig, model: nn.Module):
        r"""
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, MULTILoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, MULTILoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, MULTILoraLayer) or isinstance(module, AuxiliaryTrainingWrapper):
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        conv2d = isinstance(target, MULTILoraConv2d)
        if conv2d:

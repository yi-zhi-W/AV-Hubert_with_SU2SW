...
class MULTILoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "multilora_A_shared", "multilora_B_shared", "multilora_A", "multilora_B", "multilora_scaling", "lora_embedding_A", "lora_embedding_B")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.multilora_A_shared = nn.ModuleDict({})
        self.multilora_B_shared = nn.ModuleDict({})
...
class MULTILoraLinear(nn.Module, MULTILoraLayer):
    ...
    def update_layer(
        ...
        self.multilora_A_shared[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.multilora_B_shared[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        for adapter_name in adapter_names:
            ...
            self.multilora_A_shared[adapter_name].requires_grad_(True)
            self.multilora_B_shared[adapter_name].requires_grad_(True)
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                multilora_A_shared = self.multilora_A_shared[active_adapter]
                multilora_B_shared = self.multilora_B_shared[active_adapter]
                ...
                if not self.use_dora[active_adapter]:
                    result += multilora_B_shared(multilora_A_shared(dropout(x)))
                    for i in range(self.task_num[active_adapter]):
                        ...
                        result += multilora_B_task(multilora_A_task(dropout(x))) * self.multilora_scaling[active_adapter][i, ...].unsqueeze(0).unsqueeze(0)

...
class MULTILoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "multilora_A", "multilora_B", "lora_embedding_A", "lora_embedding_B")
...
class MULTILoraLinear(nn.Module, MULTILoraLayer):
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, task_num):
        if init_lora_weights is False:
            return
        temp_name = adapter_name+"_0"
        if temp_name in self.multilora_A.keys():
            ...
            for num in range(task_num):
                ...
                nn.init.zeros_(self.multilora_B[adapter_name+"_"+str(num)].weight)
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            find = False
            multilora_A_keys = self.multilora_A.keys()
            for active_adapter in self.active_adapters:
                temp_name = active_adapter+"_"+str(0)
                if temp_name not in multilora_A_keys:
                    continue
                find = True
                ...
                if not self.use_dora[active_adapter]:
                    for i in range(self.task_num[active_adapter]):
                        multilora_A_task = self.multilora_A[active_adapter+"_"+str(language_id.item())]
                        multilora_B_task = self.multilora_B[active_adapter+"_"+str(language_id.item())]
                        result += multilora_B_task(multilora_A_task(dropout(x)))

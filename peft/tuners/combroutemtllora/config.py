...
@dataclass
class COMBROUTEMTLLoraRuntimeConfig:
    """
...
@dataclass
class COMBROUTEMTLLoraConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`COMBROUTEMTLLoraModel`].
    ...
    B_num: int = field(default=1, metadata={"help": "B mun"})
    lambda_num: int = field(default=1, metadata={"help": "lambda mun"})
    diagonal_format: bool = field(
        default=False,
        metadata={"help": "Set this to True if the lambda to be diagonal"},
    )
    B_scale: float = field(default=0.0, metadata={"help": "B scale"})
    ...
    loftq_config: Union[COMBROUTEMTLLoftQConfig, dict] = field(
        default_factory=dict,
        metadata={
            "help": (
    ...
    runtime_config: COMBROUTEMTLLoraRuntimeConfig = field(
        default_factory=COMBROUTEMTLLoraRuntimeConfig, metadata={"help": "Runtime configurations"}
    )
    ...
    def __post_init__(self):
        super().__post_init__()
        self.peft_type = PeftType.COMBROUTEMTLLORA
        self.target_modules = (

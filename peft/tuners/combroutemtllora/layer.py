...
from .config import COMBROUTEMTLLoraConfig
from .dora import DoraConv2dLayer, DoraConv3dLayer, DoraEmbeddingLayer, DoraLinearLayer, _DoraConvNdLayer
from peft.tuners._buffer_dict import BufferDict

class COMBROUTEMTLLoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "combroutemtllora_A_shared", "combroutemtllora_B_shared", "combroutemtllora_A", "combroutemtllora_B", "combroutemtllora_lambda", "combroutemtllora_B_w", "combroutemtllora_embedding_A", "combroutemtllora_embedding_B", "combroutemtllora_gate")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.B_num = {}
        self.lambda_num = {}
        # self.combroutemtllora_lambda = nn.ModuleDict({})
        ...
        if not self.diagonal:
            ...
            self.combroutemtllora_lambda = nn.ParameterDict({})
        ...
        self.combroutemtllora_gate = nn.ModuleDict({})
        
        self.combroutemtllora_A_shared = nn.ModuleDict({})
        self.combroutemtllora_B_shared = nn.ModuleDict({})
        self.combroutemtllora_A = nn.ModuleDict({})
        self.combroutemtllora_B = nn.ModuleDict({})
        self.combroutemtllora_B_w = nn.ParameterDict({})
        # For Embedding layer
        self.combroutemtllora_embedding_A = nn.ParameterDict({})
        self.combroutemtllora_embedding_B = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        raise NotImplementedError(f"COMBROUTEMTLLoraLayer can not update_layer")

    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        raise NotImplementedError(f"COMBROUTEMTLLoraLayer can not reset_lora_parameters")
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.combroutemtllora_A.keys():
                continue
...
class COMBROUTEMTLLinear(nn.Module, COMBROUTEMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        COMBROUTEMTLLoraLayer.__init__(self, base_layer, **kwargs)
    ...
    def update_layer(
        ...
        self.combroutemtllora_A_shared[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.combroutemtllora_B_shared[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)

        self.combroutemtllora_gate[adapter_name] = nn.Linear(self.in_features, B_num, bias=lora_bias)

        self.combroutemtllora_A[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.combroutemtllora_B_w[adapter_name] =  nn.Parameter(torch.zeros(lambda_num, B_num))
        ...
        for num in range(B_num):
            combroutemtllora_B_name = adapter_name+"_"+str(num)
            self.combroutemtllora_B[combroutemtllora_B_name] = nn.Linear(r, self.out_features, bias=lora_bias)
        if not self.diagonal:
            for count in range(lambda_num):
                combroutemtllora_lambda_name = adapter_name+"_"+str(count)
                self.combroutemtllora_lambda[combroutemtllora_lambda_name] = nn.Linear(r, r, bias=lora_bias)
        else:
            self.combroutemtllora_lambda[adapter_name] = nn.Parameter(torch.zeros((lambda_num, r)))
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            nn.init.zeros_(self.combroutemtllora_B[adapter_name].weight)
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, B_num, lambda_num, device: Optional[torch.device] = None) -> None:
        ...
        for num in range(B_num):
            adapter_layer = getattr(self, "combroutemtllora_B", None)
            combroutemtllora_B_name = adapter_name+"_"+str(num)
            if combroutemtllora_B_name not in adapter_layer:
                raise ValueError("combroutemtllora_B_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[combroutemtllora_B_name] = adapter_layer[combroutemtllora_B_name].to(device, dtype=dtype)
            else:
                adapter_layer[combroutemtllora_B_name] = adapter_layer[combroutemtllora_B_name].to(device)
        if not self.diagonal:
            for count in range(lambda_num):
                adapter_layer = getattr(self, "combroutemtllora_lambda", None)
                combroutemtllora_lambda_name = adapter_name+"_"+str(count)
                if combroutemtllora_lambda_name not in adapter_layer:
                    raise ValueError("combroutemtllora_lambda_name not in adapter_layer")
                if any(p.device == meta for p in adapter_layer.parameters()):
                    continue
                if weight.dtype.is_floating_point or weight.dtype.is_complex:
                    adapter_layer[combroutemtllora_lambda_name] = adapter_layer[combroutemtllora_lambda_name].to(device, dtype=dtype)
                else:
                    adapter_layer[combroutemtllora_lambda_name] = adapter_layer[combroutemtllora_lambda_name].to(device)
        else:
            adapter_layer = getattr(self, "combroutemtllora_lambda", None)
            if adapter_name not in adapter_layer:
                raise ValueError("combroutemtllora_lambda_name not in adapter_layer")
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        # adapter_layer_names = ("combroutemtllora_A", "combroutemtllora_B", "combroutemtllora_lambda", "combroutemtllora_B_w", "combroutemtllora_embedding_A", "combroutemtllora_embedding_B")
        for layer_name in self.adapter_layer_names:
            module_dict = getattr(self, layer_name)
            for key, layer in module_dict.items():
                layer.requires_grad_(False)
        ...
        for adapter_name in adapter_names:
            self.combroutemtllora_A_shared[adapter_name].requires_grad_(True)
            self.combroutemtllora_B_shared[adapter_name].requires_grad_(True)

            self.combroutemtllora_gate[adapter_name].requires_grad_(True)

            self.combroutemtllora_A[adapter_name].requires_grad_(True)
            self.combroutemtllora_B_w[adapter_name].requires_grad_(True)
            for num in range(self.B_num[adapter_name]):
                combroutemtllora_B_name = adapter_name+"_"+str(num)
                self.combroutemtllora_B[combroutemtllora_B_name].requires_grad_(True)

            if not self.diagonal:
                for count in range(self.lambda_num[adapter_name]):
                    combroutemtllora_lambda_name = adapter_name+"_"+str(count)
                    self.combroutemtllora_lambda[combroutemtllora_lambda_name].requires_grad_(True)
            else:
                self.combroutemtllora_lambda[adapter_name].requires_grad_(True)
            ...
            # for count in range(self.lambda_num[adapter_name]):
            #     combroutemtllora_lambda_name = adapter_name+"_"+str(count)
            #     # print("combroutemtllora_lambda_name = adapter_name+str(count)")
            #     # print(combroutemtllora_lambda_name)
            #     self.combroutemtllora_lambda[combroutemtllora_lambda_name].requires_grad_(True)
            #     # for param in self.combroutemtllora_lambda[combroutemtllora_lambda_name].parameters():
            #     #     print("self.combroutemtllora_lambda[combroutemtllora_lambda_name].requires_grad")
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, B_num, lambda_num):
        ...
        if adapter_name in self.combroutemtllora_A.keys():
            if init_lora_weights is True:
                # initialize A the same way as the default for nn.Linear and B to zero
                # https://github.com/microsoft/LoRA/blob/a0a92e0f26c067cf94747bdbf1ce73793fa44d19/loralib/layers.py#L124
                nn.init.kaiming_uniform_(self.combroutemtllora_A_shared[adapter_name].weight, a=math.sqrt(5))


                nn.init.kaiming_uniform_(self.combroutemtllora_A[adapter_name].weight, a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.combroutemtllora_A_shared[adapter_name].weight, std=1 / self.r[adapter_name])

                nn.init.normal_(self.combroutemtllora_A[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_lora_weights=}")

            nn.init.zeros_(self.combroutemtllora_B_shared[adapter_name].weight)


            nn.init.kaiming_uniform_(self.combroutemtllora_B_w[adapter_name], a=math.sqrt(5))

            nn.init.kaiming_uniform_(self.combroutemtllora_gate[adapter_name].weight, a=math.sqrt(5))


            for num in range(B_num):
                nn.init.zeros_(self.combroutemtllora_B[adapter_name+"_"+str(num)].weight)

            if not self.diagonal:
                for count in range(lambda_num):
                    nn.init.eye_(self.combroutemtllora_lambda[adapter_name+"_"+str(count)].weight)
            else:
                nn.init.ones_(self.combroutemtllora_lambda[adapter_name])


            # for count in range(lambda_num):
            #     nn.init.eye_(self.combroutemtllora_lambda[adapter_name+"_"+str(count)].weight)
                # nn.init.zeros_(self.combroutemtllora_lambda[adapter_name+"_"+str(count)].weight)

            if self.lora_bias[adapter_name]:
                nn.init.zeros_(self.combroutemtllora_B[adapter_name].bias)
        if adapter_name in self.combroutemtllora_embedding_A.keys():
            # Initialize A to zeros and B the same way as the default for nn.Embedding, see:
            # https://github.com/microsoft/LoRA/blob/4c0333854cb905966f8cc4e9a74068c1e507c7b7/loralib/layers.py#L59-L60
            nn.init.zeros_(self.combroutemtllora_embedding_A[adapter_name])
            nn.init.normal_(self.combroutemtllora_embedding_B[adapter_name])
            if self.lora_bias[adapter_name]:
                # embeddings are not supported at the moment, but still adding this for consistency
                nn.init.zeros_(self.combroutemtllora_embedding_B[adapter_name].bias)

    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        raise NotImplementedError(f"COMBROUTEMTL Linear can not be merged")
        

    def unmerge(self) -> None:
        raise NotImplementedError(f"COMBROUTEMTL Linear can not be unmerged")
        

    def get_delta_weight(self, adapter) -> torch.Tensor:
        raise NotImplementedError(f"COMBROUTEMTL Linear get_delta_weight not implemented")
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        # lambda_index = kwargs.pop("language_id", None)
        lambda_index = language_id
        # if not lambda_index:
        #     raise ValueError("lambda_index shouldn't be `None`")
        if len(lambda_index)>1:
            raise ValueError("lambda_index shouldn't > 1")
        if self.disable_adapters:
            ...
            combroutemtllora_A_keys = self.combroutemtllora_A.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in combroutemtllora_A_keys:
                    continue


                if not self.diagonal:
                    combroutemtllora_A_shared = self.combroutemtllora_A_shared[active_adapter]
                    combroutemtllora_B_shared = self.combroutemtllora_B_shared[active_adapter]


                    combroutemtllora_gate = self.combroutemtllora_gate[active_adapter]
                    dropout = self.lora_dropout[active_adapter]
                    scaling = self.scaling[active_adapter]

                    router_languageid = combroutemtllora_gate(x)
                    ...
                    # column_sum = torch.sum(route_weight, dim=1)  # 结果形状 [1, 5]

                    # print("column_sum")
                    # print(column_sum)

                    # # 步骤2：压缩不必要的 batch 维度
                    # column_sum = column_sum.squeeze(0)  # 结果形状 [5]

                    # # 步骤3：归一化（确保和为1）
                    # normalized_route_weight = column_sum / torch.sum(column_sum)

                    # print("normalized_route_weight")
                    # print(normalized_route_weight)

                    combroutemtllora_A = self.combroutemtllora_A[active_adapter]
                    # print("combroutemtllora_A")
                    # print(combroutemtllora_A.weight)
                    lora_lambda = self.combroutemtllora_lambda[active_adapter+"_"+str(lambda_index.item())]

                    # for param in self.combroutemtllora_lambda[active_adapter+"_"+str(lambda_index.item())].parameters():
                    #     print("self.combroutemtllora_lambda.requires_grad")
                    #     print(param.requires_grad)
                    # print("lora_lambda")
                    # print(active_adapter+"_"+str(lambda_index.item()))
                    # print(lora_lambda.weight)
                    # all_B = []
                    # for num in range(self.B_num[active_adapter]):
                    #     combroutemtllora_B = self.combroutemtllora_B[active_adapter+"_"+str(num)]
                    #     # print("combroutemtllora_B")
                    #     # print(active_adapter+"_"+str(num))
                    #     # print(combroutemtllora_B.weight)
                    #     all_B.append(combroutemtllora_B.weight)
                    # all_B = torch.stack(all_B, dim=0)
                    # # **************************************************no combroutemtllora_B_w

                    # combroutemtllora_B_w = F.softmax(
                    #     self.combroutemtllora_B_w[active_adapter] / self.B_scale[active_adapter], 
                    #     dim=-1
                    # )
                    # # print("self.combroutemtllora_B_w[active_adapter]")
                    # # print(self.combroutemtllora_B_w[active_adapter])
                    # # print("selected_combroutemtllora_B_w")
                    # # print(selected_combroutemtllora_B_w)
                    # combroutemtllora_B_w = combroutemtllora_B_w[lambda_index.item()]
                    # # print("combroutemtllora_B_w")
                    # # print(combroutemtllora_B_w)
                    # # print(combroutemtllora_B_w.shape)

                    # combined_weight = route_weight + combroutemtllora_B_w
                    
                    combined_weight = route_weight

                    # with open("/home/wyz/projects/LLaMA-Factory/gate.txt", 'a') as f:
                    #     route_data = route_weight.squeeze(0).detach().cpu().numpy()
                    #     f.write("route_weight\n")
                    ...
                    #     f.write("combroutemtllora_B_w\n")
                    #     combroutemtllora_B_w_data = combroutemtllora_B_w.detach().cpu().numpy()
                    #     line = " ".join([f"{x:.6f}" for x in combroutemtllora_B_w_data])
                    #     f.write(line + "\n")
                    #     f.write("\n")

                    #     f.write("combined_weight\n")
                    #     combined_weightroute_data = combined_weight.squeeze(0).detach().cpu().numpy()
                    #     for row in combined_weightroute_data:
                    #         # 将每行5个数值格式化为字符串
                    #         line = " ".join([f"{x:.6f}" for x in row])
                    #         f.write(line + "\n")
                    #     f.write("\n")
                    ...
                    x = self._cast_input_dtype(x, combroutemtllora_A.weight.dtype)
                    # after_shared = combroutemtllora_B_shared(combroutemtllora_A_shared(dropout(x))) * scaling

                    after_A = combroutemtllora_A(dropout(x))
                    after_lambda = lora_lambda(after_A)
                    for i in range(self.B_num[active_adapter]):

                        result = result + self.combroutemtllora_B[active_adapter+"_"+str(i)](after_lambda) * torch.unsqueeze(combined_weight[:,:,i], -1) * scaling
                    ...
                    # print("combroutemtllora_A")
                    # print(combroutemtllora_A.weight)
                    lora_lambda = torch.index_select(self.combroutemtllora_lambda[active_adapter], 0, lambda_index)
                    lora_lambda = torch.diag_embed(lora_lambda)
                    # print("lora_lambda")
                    # print(active_adapter+"_"+str(lambda_index.item()))
                    # print(lora_lambda.weight)
                    # all_B = []
                    ...
                    # task_B = torch.einsum('lb,bor->lor', combroutemtllora_B_w, all_B)

                    # selected_combroutemtllora_B_w = task_B[lambda_index.item()]
                    # print("selected_combroutemtllora_B_w")
                    # print(selected_combroutemtllora_B_w)
                    combroutemtllora_B_w = combroutemtllora_B_w[lambda_index.item()]
                    # print("combroutemtllora_B_w")
                    ...
                    # column_sum = torch.sum(route_weight, dim=[0, 1])
                    # normalized_weights = column_sum / torch.sum(column_sum)
                    

                    combined_weight = route_weight + combroutemtllora_B_w
                    # combined_weight = torch.softmax(combined_weight, dim=-1)
                    ...
                    after_A = combroutemtllora_A(dropout(x))
                    after_lambda = torch.bmm(after_A, lora_lambda.transpose(-2, -1))
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    # after_B = after_lambda @ selected_combroutemtllora_B_w.transpose(-2, -1)
...
class COMBROUTEMTLEmbedding(nn.Module, COMBROUTEMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        COMBROUTEMTLLoraLayer.__init__(self, base_layer)
    ...
    def update_layer(
        ...
        self.combroutemtllora_embedding_A[adapter_name] = nn.Parameter(weight_A)
        self.combroutemtllora_embedding_B[adapter_name] = nn.Parameter(weight_B)
        self.lora_bias[adapter_name] = lora_bias
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.combroutemtllora_embedding_A.keys():
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        """
        device = self.combroutemtllora_embedding_B[adapter].device
        dtype = self.combroutemtllora_embedding_A[adapter].dtype
        ...
        weight_A = self.combroutemtllora_embedding_A[adapter]
        weight_B = self.combroutemtllora_embedding_B[adapter]
        ...
        if cast_to_fp32:
            ...
            self.combroutemtllora_embedding_A[adapter] = weight_A.to(dtype)
            self.combroutemtllora_embedding_B[adapter] = weight_B.to(dtype)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            if active_adapter == "__base__":
                continue
            if active_adapter not in self.combroutemtllora_embedding_A.keys():
                continue

            embedding_A = self.combroutemtllora_embedding_A[active_adapter].T
            embedding_B = self.combroutemtllora_embedding_B[active_adapter].T
            scaling = self.scaling[active_adapter]
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            torch_result_dtype = result.dtype
            for active_adapter in self.active_adapters:
                if active_adapter not in self.combroutemtllora_embedding_A:
                    continue
                embedding_A = self.combroutemtllora_embedding_A[active_adapter].T
                embedding_B = self.combroutemtllora_embedding_B[active_adapter].T
                scaling = self.scaling[active_adapter]
...
class _COMBROUTEMTLConvNd(nn.Module, COMBROUTEMTLLoraLayer):
...
class COMBROUTEMTLConv2d(_COMBROUTEMTLConvNd):
...
class COMBROUTEMTLConv1d(_COMBROUTEMTLConvNd):
...
class COMBROUTEMTLConv3d(_COMBROUTEMTLConvNd):
...
class COMBROUTEMTLMultiheadAttention(nn.Module, COMBROUTEMTLLoraLayer):
...
def dispatch_combroutemtllora(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        embedding_kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLEmbedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLConv2d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv3d):
        kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLConv3d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, nn.Conv1d):
        kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLConv1d(target, adapter_name, **kwargs)
        ...
        kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLMultiheadAttention(target, adapter_name, **kwargs)
        ...
        if kwargs["fan_in_fan_out"]:
            ...
            kwargs["fan_in_fan_out"] = combroutemtllora_config.fan_in_fan_out = False
        kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLLinear(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, Conv1D):
        if not kwargs["fan_in_fan_out"]:
            warnings.warn(
                "fan_in_fan_out is set to False but the target module is `Conv1D`. Setting fan_in_fan_out to True."
            )
            kwargs["fan_in_fan_out"] = combroutemtllora_config.fan_in_fan_out = True
        kwargs.update(combroutemtllora_config.loftq_config)
        new_module = COMBROUTEMTLLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)

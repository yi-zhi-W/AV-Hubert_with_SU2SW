...
from .layer import COMBROUTEMTLConv2d, COMBROUTEMTLLoraLayer, dispatch_combroutemtllora
...
class COMBROUTEMTLLoraModel(BaseTuner):

    prefix: str = "combroutemtllora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

        print("*********************COMBROUTEMTLLoraModel __init__****************************")

    def _check_new_adapter_config(self, config: COMBROUTEMTLLoraConfig) -> None:
        """
    ...
    @staticmethod
    def _check_target_module_exists(mtllora_config, key):
        return check_target_module_exists(mtllora_config, key)

    def _prepare_model(self, peft_config: COMBROUTEMTLLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        r_key = get_pattern_key(mtllora_config.rank_pattern.keys(), current_key)
        alpha_key = get_pattern_key(mtllora_config.alpha_pattern.keys(), current_key)
        r = mtllora_config.rank_pattern.get(r_key, mtllora_config.r)
        alpha = mtllora_config.alpha_pattern.get(alpha_key, mtllora_config.lora_alpha)

        kwargs = {
            "r": r,
            "lora_alpha": alpha,
            "lora_dropout": mtllora_config.lora_dropout,
            "B_num": mtllora_config.B_num,
            "lambda_num": mtllora_config.lambda_num,
            "diagonal_format": mtllora_config.diagonal_format,
            "B_scale": mtllora_config.B_scale,
        ...
        # note: AdaMTLLoraLayer is a subclass of MTLLoraLayer, we need to exclude it
        from peft.tuners.adalora import AdaLoraLayer

        if isinstance(target, COMBROUTEMTLLoraLayer) and not isinstance(target, AdaLoraLayer):
            raise NotImplementedError(f"isinstance(target, COMBROUTEMTLLoraLayer) and not isinstance(target, AdaCOMBROUTEMTLLoraLayer).")
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
            ...
            new_module = self._create_new_module(mtllora_config, adapter_name, target, device_map=device_map, **kwargs)
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, COMBROUTEMTLLoraLayer) and hasattr(m, "bias") and m.bias is not None:
                        m.bias.requires_grad = True
            else:
                raise NotImplementedError(f"Requested bias: {bias}, is not implemented.")

    @staticmethod
    def _create_new_module(mtllora_config, adapter_name, target, **kwargs):
        # Collect dispatcher functions to decide what backend to use for the replaced LoRA layer. The order matters,
        # because the first match is always used. Therefore, the default layers should be checked last.
        dispatchers = []

        if mtllora_config._custom_modules:
            ...
            def dynamic_dispatch_func(target, adapter_name, mtllora_config, **kwargs):
                new_module = None
                ...
                for key, custom_cls in mtllora_config._custom_modules.items():
        ...
        dispatchers.extend(
            [
                dispatch_combroutemtllora
            ]
        )

        new_module = None
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, combroutemtllora_config=mtllora_config, **kwargs)
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, COMBROUTEMTLLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, COMBROUTEMTLLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, COMBROUTEMTLLoraLayer) or isinstance(module, AuxiliaryTrainingWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, COMBROUTEMTLLoraLayer):
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        conv2d = isinstance(target, COMBROUTEMTLConv2d)
        if conv2d:

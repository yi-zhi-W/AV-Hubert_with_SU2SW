...
from .layer import HYDRALoraConv2d, HYDRALoraLayer, dispatch_HYDRALora_lora
...
class HYDRALoraModel(BaseTuner):
    ...
    prefix: str = "hydralora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

        print("*********************HYDRALoraModel __init__****************************")
    ...
    def _create_and_replace(
        ...
        if isinstance(target, HYDRALoraLayer) and not isinstance(target, AdaLoraLayer):
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, HYDRALoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, HYDRALoraLayer) or isinstance(module, AuxiliaryTrainingWrapper):
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        conv2d = isinstance(target, HYDRALoraConv2d)
        if conv2d:

...
class HYDRALoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "hydralora_A", "hydralora_B", "hydralora_A_shared", "hydralora_B_shared", "hydralora_gate", "lora_embedding_A", "lora_embedding_B")
    # adapter_layer_names = ("lora_A", "lora_B", "hydralora_A", "hydralora_B", "hydralora_gate", "lora_embedding_A", "lora_embedding_B")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.hydralora_gate = nn.ModuleDict({})
        self.hydralora_A_shared = nn.ModuleDict({})
        self.hydralora_B_shared = nn.ModuleDict({})
        self.hydralora_A = nn.ModuleDict({})
        self.hydralora_B = nn.ModuleDict({})
        # For Embedding layer
    ...
    def update_layer(
        ...
        raise NotImplementedError(f"HYDRALoraLayer can not update_layer")

    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        raise NotImplementedError(f"HYDRALoraLayer can not reset_lora_parameters")
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.hydralora_A.keys():
                continue
...
class HYDRALoraLinear(nn.Module, HYDRALoraLayer):
    ...
    def update_layer(
        ...
        self.hydralora_gate[adapter_name] = nn.Linear(self.in_features, task_num, bias=False)
        self.hydralora_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)

        self.hydralora_A_shared[adapter_name] = nn.Linear(self.in_features, r, bias=False)
        self.hydralora_B_shared[adapter_name] = nn.Linear(r, self.out_features, bias=False)
        ...
        for num in range(task_num):
            ...
            self.hydralora_B[mtllora_name] = nn.Linear(r, self.out_features, bias=False)
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            nn.init.zeros_(self.hydralora_B[adapter_name].weight)
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, task_num, device: Optional[torch.device] = None) -> None:
        ...
        for num in range(task_num):
            adapter_layer = getattr(self, "hydralora_B", None)
            hydralora_B_name = adapter_name+"_"+str(num)
            if hydralora_B_name not in adapter_layer:
                raise ValueError("hydralora_B_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[hydralora_B_name] = adapter_layer[hydralora_B_name].to(device, dtype=dtype)
            else:
                adapter_layer[hydralora_B_name] = adapter_layer[hydralora_B_name].to(device)
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        for adapter_name in adapter_names:
            self.hydralora_gate[adapter_name].requires_grad_(True)
            self.hydralora_A[adapter_name].requires_grad_(True)
            self.hydralora_A_shared[adapter_name].requires_grad_(True)
            self.hydralora_B_shared[adapter_name].requires_grad_(True)
            for num in range(self.task_num[adapter_name]):
                mtllora_name = adapter_name+"_"+str(num)
                
                self.hydralora_B[mtllora_name].requires_grad_(True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, task_num):
        ...
        if adapter_name in self.hydralora_A.keys():
            nn.init.kaiming_uniform_(self.hydralora_gate[adapter_name].weight, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.hydralora_A[adapter_name].weight, a=math.sqrt(5))

            nn.init.kaiming_uniform_(self.hydralora_A_shared[adapter_name].weight, a=math.sqrt(5))
            nn.init.zeros_(self.hydralora_B_shared[adapter_name].weight)


            for num in range(task_num):
                
                nn.init.zeros_(self.hydralora_B[adapter_name+"_"+str(num)].weight)

            if self.lora_bias[adapter_name]:
                nn.init.zeros_(self.hydralora_B[adapter_name].bias)
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            find = False
            hydralora_A_keys = self.hydralora_A.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in hydralora_A_keys:
                    continue
                find = True


                hydralora_gate = self.hydralora_gate[active_adapter]


                expert_weight = hydralora_gate(x)

                # print("expert_weight.shape")

                # print(expert_weight.shape)

                expert_weight = nn.functional.softmax(expert_weight, dim=-1).to(result.dtype)


                # with open("/home/wyz/projects/LLaMA-Factory/gate.txt", 'a') as f:
                #         route_data = expert_weight.squeeze(0).detach().cpu().numpy()
                ...
                hydralora_A = self.hydralora_A[active_adapter]


                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                x = self._cast_input_dtype(x, hydralora_A.weight.dtype)
                if not self.use_dora[active_adapter]:
                    for i in range(self.task_num[active_adapter]):
                        
                        hydralora_B_task = self.hydralora_B[active_adapter+"_"+str(i)]
                        result += hydralora_B_task(hydralora_A(dropout(x))) * torch.unsqueeze(expert_weight[:,:,i], -1)

                    hydralora_A_shared = self.hydralora_A_shared[active_adapter]
                    hydralora_B_shared = self.hydralora_B_shared[active_adapter]
                    result += hydralora_B_shared(hydralora_A_shared(dropout(x)))
            ...
            if not find:
                raise ValueError("active_adapter not in hydralora_A_keys")
    ...
    def __repr__(self) -> str:
        rep = super().__repr__()
        return "hydralora_." + rep


class HYDRALoraEmbedding(nn.Module, HYDRALoraLayer):
...
class _HYDRALoraConvNd(nn.Module, HYDRALoraLayer):
...
class HYDRALoraConv2d(_HYDRALoraConvNd):
...
class HYDRALoraConv1d(_HYDRALoraConvNd):
...
class HYDRALoraConv3d(_HYDRALoraConvNd):
...
class HYDRALoraMultiheadAttention(nn.Module, HYDRALoraLayer):
    """LoRA implemented in a hydrahead attention layer
...
def dispatch_HYDRALora_lora(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        new_module = HYDRALoraEmbedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = HYDRALoraConv2d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv3d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = HYDRALoraConv3d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, nn.Conv1d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = HYDRALoraConv1d(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = HYDRALoraMultiheadAttention(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = HYDRALoraLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)

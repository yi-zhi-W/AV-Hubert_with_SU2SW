...
class GATEMTLLoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "gatemtllora_A_shared", "gatemtllora_B_shared", "gatemtllora_A", "gatemtllora_B", "gatemtllora_B_w", "gatemtllora_embedding_A", "gatemtllora_embedding_B", "gatemtllora_language_gate", "gatemtllora_A_languageid", "gatemtllora_B_languageid")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.gatemtllora_B = nn.ParameterDict({})
...
class GATEMTLLinear(nn.Module, GATEMTLLoraLayer):
    ...
    def update_layer(
        ...
        self.gatemtllora_B[adapter_name] = nn.Parameter(torch.zeros((B_num, self.out_features, r)))
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        for adapter_name in adapter_names:
            ...
            self.gatemtllora_B[adapter_name].requires_grad_(True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, B_num, lambda_num):
        ...
        if adapter_name in self.gatemtllora_A.keys():
            ...
            nn.init.zeros_(self.gatemtllora_B[adapter_name])
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                if active_adapter not in gatemtllora_A_keys:
                    ...
                    router_languageid = self.gatemtllora_language_gate[active_adapter](result + self.gatemtllora_B_languageid[active_adapter](self.gatemtllora_A_languageid[active_adapter](dropout(x))) * scaling)

                    route_weight = nn.functional.softmax(router_languageid, dim=-1).to(result.dtype)

                    route_weight = route_weight.mean(dim=1, keepdim=True)
                    ...
                    lora_B_w = route_weight[0] + gatemtllora_B_w[lambda_index.item()].unsqueeze(0)
                    task_B = torch.einsum('lb,bor->lor', B_w, all_B)

                    task_B = lora_B_w @ self.lora_B.view((B_num, -1))

                    task_B = task_B.reshape((-1, out_features, r))

                    result = result +  torch.bmm(self.gatemtllora_A[active_adapter](dropout(x)), task_B.transpose(-2, -1))* scaling

                    result = result + self.gatemtllora_B_shared[active_adapter](self.gatemtllora_A_shared[active_adapter](dropout(x))) * scaling

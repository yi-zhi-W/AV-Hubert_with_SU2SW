...
class GATEMTLLoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "gatemtllora_A", "gatemtllora_B", "gatemtllora_lambda", "gatemtllora_B_w", "gatemtllora_embedding_A", "gatemtllora_embedding_B", "gatemtllora_language_gate", "gatemtllora_A_languageid", "gatemtllora_B_languageid")
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.diagonal:
                    ...
                    # for param in gatemtllora_A_languageid.parameters():
                    #     print("self.gatemtllora_A_languageid[active_adapter].requires_grad")
                    #     print(param.requires_grad)
                    # for param in gatemtllora_B_languageid.parameters():
                    #     print("self.gatemtllora_B_languageid[active_adapter].requires_grad")
                    #     print(param.requires_grad)
                    ...
                    # print("gatemtllora_A")
                    # print(gatemtllora_A.weight)
                    lora_lambda = self.gatemtllora_lambda[active_adapter+"_"+str(lambda_index.item())]
                    # print("lora_lambda")
                    # print(active_adapter+"_"+str(lambda_index.item()))
                    # print(lora_lambda.weight)
                    # all_B = []
                    # for num in range(self.B_num[active_adapter]):
                    #     gatemtllora_B = self.gatemtllora_B[active_adapter+"_"+str(num)]
                    #     # print("gatemtllora_B")
                    #     # print(active_adapter+"_"+str(num))
                    #     # print(gatemtllora_B.weight)
                    #     all_B.append(gatemtllora_B.weight)
                    ...
                    # print("self.gatemtllora_B_w[active_adapter]")
                    # print(self.gatemtllora_B_w[active_adapter])
                    # print("selected_gatemtllora_B_w")
                    # print(selected_gatemtllora_B_w)
                    gatemtllora_B_w = gatemtllora_B_w[lambda_index.item()]
                    # print("gatemtllora_B_w")
                    # print(gatemtllora_B_w)
                    # print(gatemtllora_B_w.shape)
                    ...
                    #     f.write("gatemtllora_B_w\n")
                    #     gatemtllora_B_w_data = gatemtllora_B_w.detach().cpu().numpy()
                    #     line = " ".join([f"{x:.6f}" for x in gatemtllora_B_w_data])
                    #     f.write(line + "\n")
                    #     f.write("\n")
                    ...
                    for i in range(self.B_num[active_adapter]):
                        ...
                        result = result + self.gatemtllora_B[active_adapter+"_"+str(i)](after_lambda) * torch.unsqueeze(combined_weight[:,:,i], -1) * scaling

                else:
                    if not self.no_lambda:
                        ...
                        # print("gatemtllora_A")
                        # print(gatemtllora_A.weight)
                        lora_lambda = torch.index_select(self.gatemtllora_lambda[active_adapter], 0, lambda_index)


                        lora_lambda = torch.diag_embed(lora_lambda)

                        # device = gatemtllora_A.device
                        # diag_vector = torch.ones(5, device=device)
                        # lora_lambda = torch.diag_embed(diag_vector)
                        ...
                        # task_B = torch.einsum('lb,bor->lor', gatemtllora_B_w, all_B)

                        # selected_gatemtllora_B_w = task_B[lambda_index.item()]
                        # print("selected_gatemtllora_B_w")
                        # print(selected_gatemtllora_B_w)
                        gatemtllora_B_w = gatemtllora_B_w[lambda_index.item()]
                        # print("gatemtllora_B_w")
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    # after_B = after_lambda @ selected_gatemtllora_B_w.transpose(-2, -1)

...
from .layer import COMSCALMTLLoraConv2d, COMSCALMTLLoraLayer, dispatch_comscalmtllora
...
class COMSCALMTLLoraModel(BaseTuner):

    prefix: str = "comscalmtllora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

        print("*********************COMSCALMTLLoraModel __init__****************************")

    def _check_new_adapter_config(self, config: COMSCALMTLLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: COMSCALMTLLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, COMSCALMTLLoraLayer) and not isinstance(target, AdaLoraLayer):
            raise NotImplementedError(f"isinstance(target, COMSCALMTLLoraLayer) and not isinstance(target, AdaCOMSCALMTLLoraLayer).")
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, COMSCALMTLLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def _create_new_module(mtllora_config, adapter_name, target, **kwargs):
        ...
        dispatchers.extend(
            [
                dispatch_comscalmtllora
            ]
        )

        new_module = None
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, comscalmtllora_config=mtllora_config, **kwargs)
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, COMSCALMTLLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, COMSCALMTLLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, COMSCALMTLLoraLayer) or isinstance(module, AuxiliaryTrainingWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, COMSCALMTLLoraLayer):
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        conv2d = isinstance(target, COMSCALMTLLoraConv2d)
        if conv2d:

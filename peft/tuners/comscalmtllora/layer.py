...
from .config import COMSCALMTLLoraConfig
from .dora import DoraConv2dLayer, DoraConv3dLayer, DoraEmbeddingLayer, DoraLinearLayer, _DoraConvNdLayer
from peft.tuners._buffer_dict import BufferDict

class COMSCALMTLLoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "comscalmtllora_A_shared", "comscalmtllora_B_shared", "comscalmtllora_A", "comscalmtllora_B", "comscalmtllora_lambda", "comscalmtllora_route", "comscalmtllora_embedding_A", "comscalmtllora_embedding_B", "B_lora_scaling")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.B_num = {}
        self.lambda_num = {}
        self.comscalmtllora_lambda = nn.ModuleDict({})
        self.diagonal_format = {}
        self.B_scale = {}
        self.B_lora_scaling = nn.ParameterDict({})
        self.lora_dropout = nn.ModuleDict({})
        self.lora_A = nn.ModuleDict({})
        self.lora_B = nn.ModuleDict({})
        self.comscalmtllora_A_shared = nn.ModuleDict({})
        self.comscalmtllora_B_shared = nn.ModuleDict({})
        self.comscalmtllora_A = nn.ModuleDict({})
        self.comscalmtllora_B = nn.ModuleDict({})
        self.comscalmtllora_route = nn.ModuleDict({})
        # For Embedding layer
        self.comscalmtllora_embedding_A = nn.ParameterDict({})
        self.comscalmtllora_embedding_B = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        raise NotImplementedError(f"COMSCALMTLLoraLayer can not update_layer")

    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        raise NotImplementedError(f"COMSCALMTLLoraLayer can not reset_lora_parameters")
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.comscalmtllora_A.keys():
                continue
...
class COMSCALMTLLoraLinear(nn.Module, COMSCALMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        COMSCALMTLLoraLayer.__init__(self, base_layer, **kwargs)
    ...
    def update_layer(
        ...
        self.comscalmtllora_A_shared[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.comscalmtllora_B_shared[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)

        self.comscalmtllora_A[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.comscalmtllora_route[adapter_name] =  nn.Linear(self.in_features, B_num, bias=False)
        self.B_lora_scaling[adapter_name] =  nn.Parameter(torch.zeros(B_num, self.out_features))
        ...
        for num in range(B_num):
            comscalmtllora_B_name = adapter_name+"_"+str(num)
            self.comscalmtllora_B[comscalmtllora_B_name] = nn.Linear(r, self.out_features, bias=lora_bias)
        for count in range(lambda_num):
            comscalmtllora_lambda_name = adapter_name+"_"+str(count)
            self.comscalmtllora_lambda[comscalmtllora_lambda_name] = nn.Linear(r, r, bias=lora_bias)
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            nn.init.zeros_(self.comscalmtllora_B[adapter_name].weight)
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, B_num, lambda_num, device: Optional[torch.device] = None) -> None:
        ...
        for num in range(B_num):
            adapter_layer = getattr(self, "comscalmtllora_B", None)
            comscalmtllora_B_name = adapter_name+"_"+str(num)
            if comscalmtllora_B_name not in adapter_layer:
                raise ValueError("comscalmtllora_B_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[comscalmtllora_B_name] = adapter_layer[comscalmtllora_B_name].to(device, dtype=dtype)
            else:
                adapter_layer[comscalmtllora_B_name] = adapter_layer[comscalmtllora_B_name].to(device)

        for count in range(lambda_num):
            adapter_layer = getattr(self, "comscalmtllora_lambda", None)
            comscalmtllora_lambda_name = adapter_name+"_"+str(count)
            if comscalmtllora_lambda_name not in adapter_layer:
                raise ValueError("comscalmtllora_lambda_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[comscalmtllora_lambda_name] = adapter_layer[comscalmtllora_lambda_name].to(device, dtype=dtype)
            else:
                adapter_layer[comscalmtllora_lambda_name] = adapter_layer[comscalmtllora_lambda_name].to(device)
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        for adapter_name in adapter_names:
            self.comscalmtllora_A_shared[adapter_name].requires_grad_(True)
            self.comscalmtllora_B_shared[adapter_name].requires_grad_(True)

            self.comscalmtllora_A[adapter_name].requires_grad_(True)
            self.comscalmtllora_route[adapter_name].requires_grad_(True)
            self.B_lora_scaling[adapter_name].requires_grad_(True)
            for num in range(self.B_num[adapter_name]):
                comscalmtllora_B_name = adapter_name+"_"+str(num)
                self.comscalmtllora_B[comscalmtllora_B_name].requires_grad_(True)
            for count in range(self.lambda_num[adapter_name]):
                comscalmtllora_lambda_name = adapter_name+"_"+str(count)
                # print("comscalmtllora_lambda_name = adapter_name+str(count)")
                # print(comscalmtllora_lambda_name)
                self.comscalmtllora_lambda[comscalmtllora_lambda_name].requires_grad_(True)
                # for param in self.comscalmtllora_lambda[comscalmtllora_lambda_name].parameters():
                #     print("self.comscalmtllora_lambda[comscalmtllora_lambda_name].requires_grad")
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, B_num, lambda_num):
        ...
        if adapter_name in self.comscalmtllora_A.keys():
            if init_lora_weights is True:
                # initialize A the same way as the default for nn.Linear and B to zero
                # https://github.com/microsoft/LoRA/blob/a0a92e0f26c067cf94747bdbf1ce73793fa44d19/loralib/layers.py#L124
                nn.init.kaiming_uniform_(self.comscalmtllora_A_shared[adapter_name].weight, a=math.sqrt(5))


                nn.init.kaiming_uniform_(self.comscalmtllora_A[adapter_name].weight, a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.comscalmtllora_A_shared[adapter_name].weight, std=1 / self.r[adapter_name])

                nn.init.normal_(self.comscalmtllora_A[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_lora_weights=}")

            nn.init.zeros_(self.comscalmtllora_B_shared[adapter_name].weight)

            nn.init.kaiming_uniform_(self.comscalmtllora_route[adapter_name].weight, a=math.sqrt(5))

            nn.init.zeros_(self.B_lora_scaling[adapter_name])


            for num in range(B_num):
                nn.init.kaiming_uniform_(self.comscalmtllora_B[adapter_name+"_"+str(num)].weight, a=math.sqrt(5))
            for count in range(lambda_num):
                nn.init.eye_(self.comscalmtllora_lambda[adapter_name+"_"+str(count)].weight)
                # nn.init.zeros_(self.comscalmtllora_lambda[adapter_name+"_"+str(count)].weight)

            if self.lora_bias[adapter_name]:
                nn.init.zeros_(self.comscalmtllora_B[adapter_name].bias)
        if adapter_name in self.comscalmtllora_embedding_A.keys():
            # Initialize A to zeros and B the same way as the default for nn.Embedding, see:
            # https://github.com/microsoft/LoRA/blob/4c0333854cb905966f8cc4e9a74068c1e507c7b7/loralib/layers.py#L59-L60
            nn.init.zeros_(self.comscalmtllora_embedding_A[adapter_name])
            nn.init.normal_(self.comscalmtllora_embedding_B[adapter_name])
            if self.lora_bias[adapter_name]:
                # embeddings are not supported at the moment, but still adding this for consistency
                nn.init.zeros_(self.comscalmtllora_embedding_B[adapter_name].bias)

    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        raise NotImplementedError(f"COMSCALMTLLora Linear can not be merged")
        

    def unmerge(self) -> None:
        raise NotImplementedError(f"COMSCALMTLLora Linear can not be unmerged")
        

    def get_delta_weight(self, adapter) -> torch.Tensor:
        raise NotImplementedError(f"COMSCALMTLLora Linear get_delta_weight not implemented")
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            comscalmtllora_A_keys = self.comscalmtllora_A.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in comscalmtllora_A_keys:
                    continue

                comscalmtllora_A_shared = self.comscalmtllora_A_shared[active_adapter]
                comscalmtllora_B_shared = self.comscalmtllora_B_shared[active_adapter]


                comscalmtllora_A = self.comscalmtllora_A[active_adapter]
                # print("comscalmtllora_A")
                # print(comscalmtllora_A.weight)
                lora_lambda = self.comscalmtllora_lambda[active_adapter+"_"+str(lambda_index.item())]
                # print("lora_lambda")
                # print(active_adapter+"_"+str(lambda_index.item()))
                # print(lora_lambda.weight)
                all_B = []
                for num in range(self.B_num[active_adapter]):
                    comscalmtllora_B = self.comscalmtllora_B[active_adapter+"_"+str(num)]
                    # print("comscalmtllora_B")
                    # print(active_adapter+"_"+str(num))
                    # print(comscalmtllora_B.weight)
                    all_B.append(comscalmtllora_B.weight)
                all_B = torch.stack(all_B, dim=0)


                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                B_lora_scaling = self.B_lora_scaling[active_adapter]
                comscalmtllora_route = self.comscalmtllora_route[active_adapter]
                x = self._cast_input_dtype(x, comscalmtllora_A.weight.dtype)

                if not self.use_dora[active_adapter]:

                    after_shared = comscalmtllora_B_shared(comscalmtllora_A_shared(dropout(x))) * scaling

                    route_weight = nn.functional.softmax(comscalmtllora_route(x), dim=-1).to(result.dtype)

                    for i in range(self.B_num[active_adapter]):

                        result = result + self.comscalmtllora_B[active_adapter+"_"+str(i)](lora_lambda(comscalmtllora_A(dropout(x)))) * torch.unsqueeze(route_weight[:,:,i], -1) * B_lora_scaling[i, ...].unsqueeze(0).unsqueeze(0)
...
class COMSCALMTLLoraEmbedding(nn.Module, COMSCALMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        COMSCALMTLLoraLayer.__init__(self, base_layer)
    ...
    def update_layer(
        ...
        self.comscalmtllora_embedding_A[adapter_name] = nn.Parameter(weight_A)
        self.comscalmtllora_embedding_B[adapter_name] = nn.Parameter(weight_B)
        self.lora_bias[adapter_name] = lora_bias
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.comscalmtllora_embedding_A.keys():
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        """
        device = self.comscalmtllora_embedding_B[adapter].device
        dtype = self.comscalmtllora_embedding_A[adapter].dtype
        ...
        weight_A = self.comscalmtllora_embedding_A[adapter]
        weight_B = self.comscalmtllora_embedding_B[adapter]
        ...
        if cast_to_fp32:
            ...
            self.comscalmtllora_embedding_A[adapter] = weight_A.to(dtype)
            self.comscalmtllora_embedding_B[adapter] = weight_B.to(dtype)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            if active_adapter == "__base__":
                continue
            if active_adapter not in self.comscalmtllora_embedding_A.keys():
                continue

            embedding_A = self.comscalmtllora_embedding_A[active_adapter].T
            embedding_B = self.comscalmtllora_embedding_B[active_adapter].T
            scaling = self.scaling[active_adapter]
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            torch_result_dtype = result.dtype
            for active_adapter in self.active_adapters:
                if active_adapter not in self.comscalmtllora_embedding_A:
                    continue
                embedding_A = self.comscalmtllora_embedding_A[active_adapter].T
                embedding_B = self.comscalmtllora_embedding_B[active_adapter].T
                scaling = self.scaling[active_adapter]
...
class _COMSCALMTLLoraConvNd(nn.Module, COMSCALMTLLoraLayer):
...
class COMSCALMTLLoraConv2d(_COMSCALMTLLoraConvNd):
...
class COMSCALMTLLoraConv1d(_COMSCALMTLLoraConvNd):
...
class COMSCALMTLLoraConv3d(_COMSCALMTLLoraConvNd):
...
class COMSCALMTLLoraMultiheadAttention(nn.Module, COMSCALMTLLoraLayer):
...
def dispatch_comscalmtllora(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        embedding_kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraEmbedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraConv2d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv3d):
        kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraConv3d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, nn.Conv1d):
        kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraConv1d(target, adapter_name, **kwargs)
        ...
        kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraMultiheadAttention(target, adapter_name, **kwargs)
        ...
        if kwargs["fan_in_fan_out"]:
            ...
            kwargs["fan_in_fan_out"] = comscalmtllora_config.fan_in_fan_out = False
        kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraLinear(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, Conv1D):
        if not kwargs["fan_in_fan_out"]:
            warnings.warn(
                "fan_in_fan_out is set to False but the target module is `Conv1D`. Setting fan_in_fan_out to True."
            )
            kwargs["fan_in_fan_out"] = comscalmtllora_config.fan_in_fan_out = True
        kwargs.update(comscalmtllora_config.loftq_config)
        new_module = COMSCALMTLLoraLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)

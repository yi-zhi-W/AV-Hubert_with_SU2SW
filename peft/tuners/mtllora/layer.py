...
class MTLLoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "mtllora_A", "mtllora_B", "mtllora_lambda", "mtllora_B_w", "mtllora_embedding_A", "mtllora_embedding_B")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        if not self.diagonal:
            self.mtllora_lambda = nn.ModuleDict({})
        else:
            self.mtllora_lambda = nn.ParameterDict({})

        self.B_scale = {}
        self.lora_dropout = nn.ModuleDict({})
        self.lora_A = nn.ModuleDict({})
        self.lora_B = nn.ModuleDict({})
        self.mtllora_A = nn.ModuleDict({})
        self.mtllora_B = nn.ModuleDict({})
        self.mtllora_B_w = nn.ParameterDict({})
        # For Embedding layer
        self.mtllora_embedding_A = nn.ParameterDict({})
        self.mtllora_embedding_B = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        raise NotImplementedError(f"MTLLoraLayer can not update_layer")

    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        raise NotImplementedError(f"MTLLoraLayer can not reset_lora_parameters")
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.mtllora_A.keys():
                continue
...
class MTLLinear(nn.Module, MTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        MTLLoraLayer.__init__(self, base_layer, **kwargs)
    ...
    def update_layer(
        ...
        self.mtllora_A[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.mtllora_B_w[adapter_name] =  nn.Parameter(torch.zeros(lambda_num, B_num))
        ...
        for num in range(B_num):
            mtllora_B_name = adapter_name+"_"+str(num)
            self.mtllora_B[mtllora_B_name] = nn.Linear(r, self.out_features, bias=lora_bias)

        if not self.diagonal:
            for count in range(lambda_num):
                mtllora_lambda_name = adapter_name+"_"+str(count)
                self.mtllora_lambda[mtllora_lambda_name] = nn.Linear(r, r, bias=lora_bias)
        else:
            self.mtllora_lambda[adapter_name] = nn.Parameter(torch.zeros((lambda_num, r)))
            
            # self.mtllora_lambda[mtllora_lambda_name] = nn.Parameter(self.weight.new_zeros((lambda_num, r)))
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            nn.init.zeros_(self.mtllora_B[adapter_name].weight)
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, B_num, lambda_num, device: Optional[torch.device] = None) -> None:
        ...
        for num in range(B_num):
            adapter_layer = getattr(self, "mtllora_B", None)
            mtllora_B_name = adapter_name+"_"+str(num)
            if mtllora_B_name not in adapter_layer:
                raise ValueError("mtllora_B_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[mtllora_B_name] = adapter_layer[mtllora_B_name].to(device, dtype=dtype)
            else:
                adapter_layer[mtllora_B_name] = adapter_layer[mtllora_B_name].to(device)

        if not self.diagonal:
            for count in range(lambda_num):
                adapter_layer = getattr(self, "mtllora_lambda", None)
                mtllora_lambda_name = adapter_name+"_"+str(count)
                if mtllora_lambda_name not in adapter_layer:
                    raise ValueError("mtllora_lambda_name not in adapter_layer")
                if any(p.device == meta for p in adapter_layer.parameters()):
                    continue
                if weight.dtype.is_floating_point or weight.dtype.is_complex:
                    adapter_layer[mtllora_lambda_name] = adapter_layer[mtllora_lambda_name].to(device, dtype=dtype)
                else:
                    adapter_layer[mtllora_lambda_name] = adapter_layer[mtllora_lambda_name].to(device)
        else:
            adapter_layer = getattr(self, "mtllora_lambda", None)
            mtllora_lambda_name = adapter_name
            if mtllora_lambda_name not in adapter_layer:
                raise ValueError("mtllora_lambda_name not in adapter_layer")
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[mtllora_lambda_name] = adapter_layer[mtllora_lambda_name].to(device, dtype=dtype)
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        # adapter_layer_names = ("mtllora_A", "mtllora_B", "mtllora_lambda", "mtllora_B_w", "mtllora_embedding_A", "mtllora_embedding_B")
        for layer_name in self.adapter_layer_names:
            module_dict = getattr(self, layer_name)
            for key, layer in module_dict.items():
                layer.requires_grad_(False)
        ...
        for adapter_name in adapter_names:
            self.mtllora_A[adapter_name].requires_grad_(True)
            self.mtllora_B_w[adapter_name].requires_grad_(True)
            for num in range(self.B_num[adapter_name]):
                mtllora_B_name = adapter_name+"_"+str(num)
                self.mtllora_B[mtllora_B_name].requires_grad_(True)

            if not self.diagonal:

                for count in range(self.lambda_num[adapter_name]):
                    mtllora_lambda_name = adapter_name+"_"+str(count)
                    # print("mtllora_lambda_name = adapter_name+str(count)")
                    # print(mtllora_lambda_name)
                    self.mtllora_lambda[mtllora_lambda_name].requires_grad_(True)
                    # for param in self.mtllora_lambda[mtllora_lambda_name].parameters():
                    #     print("self.mtllora_lambda[mtllora_lambda_name].requires_grad")
                    #     print(param.requires_grad)
            else:
                self.mtllora_lambda[adapter_name].requires_grad_(True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, B_num, lambda_num):
        ...
        if adapter_name in self.mtllora_A.keys():
            if init_lora_weights is True:
                # initialize A the same way as the default for nn.Linear and B to zero
                # https://github.com/microsoft/LoRA/blob/a0a92e0f26c067cf94747bdbf1ce73793fa44d19/loralib/layers.py#L124
                nn.init.kaiming_uniform_(self.mtllora_A[adapter_name].weight, a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.mtllora_A[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_lora_weights=}")

            nn.init.kaiming_uniform_(self.mtllora_B_w[adapter_name], a=math.sqrt(5))
            for num in range(B_num):
                nn.init.zeros_(self.mtllora_B[adapter_name+"_"+str(num)].weight)
            if not self.diagonal:
                for count in range(lambda_num):
                    nn.init.eye_(self.mtllora_lambda[adapter_name+"_"+str(count)].weight)
            else:
                nn.init.ones_(self.mtllora_lambda[adapter_name])
                # nn.init.eye_(self.mtllora_lambda[adapter_name+"_"+str(count)].weight)
                # nn.init.zeros_(self.mtllora_lambda[adapter_name+"_"+str(count)].weight)

            if self.lora_bias[adapter_name]:
                nn.init.zeros_(self.mtllora_B[adapter_name].bias)
        if adapter_name in self.mtllora_embedding_A.keys():
            # Initialize A to zeros and B the same way as the default for nn.Embedding, see:
            # https://github.com/microsoft/LoRA/blob/4c0333854cb905966f8cc4e9a74068c1e507c7b7/loralib/layers.py#L59-L60
            nn.init.zeros_(self.mtllora_embedding_A[adapter_name])
            nn.init.normal_(self.mtllora_embedding_B[adapter_name])
            if self.lora_bias[adapter_name]:
                # embeddings are not supported at the moment, but still adding this for consistency
                nn.init.zeros_(self.mtllora_embedding_B[adapter_name].bias)
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            mtllora_A_keys = self.mtllora_A.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in mtllora_A_keys:
                    continue
                if not self.diagonal:
                    mtllora_A = self.mtllora_A[active_adapter]
                    # print("mtllora_A")
                    # print(mtllora_A.weight)
                    lora_lambda = self.mtllora_lambda[active_adapter+"_"+str(lambda_index.item())]
                    # print("lora_lambda")
                    # print(active_adapter+"_"+str(lambda_index.item()))
                    # print(lora_lambda.weight)
                    all_B = []
                    for num in range(self.B_num[active_adapter]):
                        mtllora_B = self.mtllora_B[active_adapter+"_"+str(num)]
                        # print("mtllora_B")
                        # print(active_adapter+"_"+str(num))
                        # print(mtllora_B.weight)
                        all_B.append(mtllora_B.weight)
                    all_B = torch.stack(all_B, dim=0)


                    mtllora_B_w = F.softmax(
                        self.mtllora_B_w[active_adapter] / self.B_scale[active_adapter], 
                        dim=-1
                    )
                    # print("self.mtllora_B_w[active_adapter]")
                    # print(self.mtllora_B_w[active_adapter])

                    task_B = torch.einsum('lb,bor->lor', mtllora_B_w, all_B)

                    selected_mtllora_B_w = task_B[lambda_index.item()]
                    # print("selected_mtllora_B_w")
                    # print(selected_mtllora_B_w)

                    dropout = self.lora_dropout[active_adapter]
                    scaling = self.scaling[active_adapter]
                    x = self._cast_input_dtype(x, mtllora_A.weight.dtype)
                    after_A = mtllora_A(dropout(x))
                    after_lambda = lora_lambda(after_A)
                    after_B = after_lambda @ selected_mtllora_B_w.transpose(-2, -1)
                else:
                    mtllora_A = self.mtllora_A[active_adapter]

                    lora_lambda = torch.index_select(self.mtllora_lambda[active_adapter], 0, lambda_index.item())
                    ...
                    after_B = torch.bmm(after_lambda, selected_mtllora_B_w.transpose(-2, -1))
    ...
    #             print("mtllora_A.weight")
    #             print(mtllora_A.weight)
    #             mtllora_B = self.mtllora_B[active_adapter+"_0"]
    #             print("mtllora_B.weight")
    #             print(mtllora_B.weight)
    #             dropout = self.lora_dropout[active_adapter]
    #             scaling = self.scaling[active_adapter]
    ...
    #             if not self.use_dora[active_adapter]:
    #                 result = result + mtllora_B(mtllora_A(dropout(x))) * scaling
    #             else:
    #                 if isinstance(dropout, nn.Identity) or not self.training:
    #                     base_result = result
    #                 else:
...
class MTLEmbedding(nn.Module, MTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        MTLLoraLayer.__init__(self, base_layer)
    ...
    def update_layer(
        ...
        self.mtllora_embedding_A[adapter_name] = nn.Parameter(weight_A)
        self.mtllora_embedding_B[adapter_name] = nn.Parameter(weight_B)
        self.lora_bias[adapter_name] = lora_bias
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.mtllora_embedding_A.keys():
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        """
        device = self.mtllora_embedding_B[adapter].device
        dtype = self.mtllora_embedding_A[adapter].dtype
        ...
        weight_A = self.mtllora_embedding_A[adapter]
        weight_B = self.mtllora_embedding_B[adapter]
        ...
        if cast_to_fp32:
            ...
            self.mtllora_embedding_A[adapter] = weight_A.to(dtype)
            self.mtllora_embedding_B[adapter] = weight_B.to(dtype)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            if active_adapter == "__base__":
                continue
            if active_adapter not in self.mtllora_embedding_A.keys():
                continue

            embedding_A = self.mtllora_embedding_A[active_adapter].T
            embedding_B = self.mtllora_embedding_B[active_adapter].T
            scaling = self.scaling[active_adapter]
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            torch_result_dtype = result.dtype
            for active_adapter in self.active_adapters:
                if active_adapter not in self.mtllora_embedding_A:
                    continue
                embedding_A = self.mtllora_embedding_A[active_adapter].T
                embedding_B = self.mtllora_embedding_B[active_adapter].T
                scaling = self.scaling[active_adapter]
...
class _MTLConvNd(nn.Module, MTLLoraLayer):
...
class MTLMultiheadAttention(nn.Module, MTLLoraLayer):
...
def dispatch_mtllora(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        new_module = MTLEmbedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = MTLConv2d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv3d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = MTLConv3d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, nn.Conv1d):
        kwargs.update(mtllora_config.loftq_config)
        new_module = MTLConv1d(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = MTLMultiheadAttention(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = MTLLinear(target, adapter_name, **kwargs)
        ...
        kwargs.update(mtllora_config.loftq_config)
        new_module = MTLLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)

...
from .config import COMBINEDMTLLoraConfig
from .dora import DoraConv2dLayer, DoraConv3dLayer, DoraEmbeddingLayer, DoraLinearLayer, _DoraConvNdLayer
from peft.tuners._buffer_dict import BufferDict

class COMBINEDMTLLoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "combinedmtllora_A_shared", "combinedmtllora_B_shared", "combinedmtllora_A", "combinedmtllora_B",  "combinedmtllora_B_w", "combinedmtllora_embedding_A", "combinedmtllora_embedding_B")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.combinedmtllora_A_shared = nn.ModuleDict({})
        self.combinedmtllora_B_shared = nn.ModuleDict({})
        self.combinedmtllora_A = nn.ModuleDict({})
        self.combinedmtllora_B = nn.ModuleDict({})
        self.combinedmtllora_B_w = nn.ParameterDict({})
        # For Embedding layer
        self.combinedmtllora_embedding_A = nn.ParameterDict({})
        self.combinedmtllora_embedding_B = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        raise NotImplementedError(f"COMBINEDMTLLoraLayer can not update_layer")

    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        raise NotImplementedError(f"COMBINEDMTLLoraLayer can not reset_lora_parameters")
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.combinedmtllora_A.keys():
                continue
...
class COMBINEDMTLLinear(nn.Module, COMBINEDMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        COMBINEDMTLLoraLayer.__init__(self, base_layer, **kwargs)
        self.fan_in_fan_out = fan_in_fan_out
        self.diagonal_format = False
    ...
    def update_layer(
        ...
        self.combinedmtllora_A_shared[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.combinedmtllora_B_shared[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)

        self.combinedmtllora_A[adapter_name] = nn.Linear(self.in_features, r, bias=lora_bias)
        self.combinedmtllora_B_w[adapter_name] =  nn.Parameter(torch.zeros(lambda_num, B_num))
        ...
        for num in range(B_num):
            combinedmtllora_B_name = adapter_name+"_"+str(num)
            self.combinedmtllora_B[combinedmtllora_B_name] = nn.Linear(r, self.out_features, bias=lora_bias)
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            nn.init.zeros_(self.combinedmtllora_B[adapter_name].weight)
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, B_num, lambda_num, device: Optional[torch.device] = None) -> None:
        ...
        for num in range(B_num):
            adapter_layer = getattr(self, "combinedmtllora_B", None)
            combinedmtllora_B_name = adapter_name+"_"+str(num)
            if combinedmtllora_B_name not in adapter_layer:
                raise ValueError("combinedmtllora_B_name not in adapter_layer")
            if any(p.device == meta for p in adapter_layer.parameters()):
                continue
            if weight.dtype.is_floating_point or weight.dtype.is_complex:
                adapter_layer[combinedmtllora_B_name] = adapter_layer[combinedmtllora_B_name].to(device, dtype=dtype)
            else:
                adapter_layer[combinedmtllora_B_name] = adapter_layer[combinedmtllora_B_name].to(device)
    ...
    def set_adapter(self, adapter_names: str | list[str]) -> None:
        ...
        for adapter_name in adapter_names:
            self.combinedmtllora_A_shared[adapter_name].requires_grad_(True)
            self.combinedmtllora_B_shared[adapter_name].requires_grad_(True)

            self.combinedmtllora_A[adapter_name].requires_grad_(True)
            self.combinedmtllora_B_w[adapter_name].requires_grad_(True)
            for num in range(self.B_num[adapter_name]):
                combinedmtllora_B_name = adapter_name+"_"+str(num)
                self.combinedmtllora_B[combinedmtllora_B_name].requires_grad_(True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, B_num, lambda_num):
        ...
        if adapter_name in self.combinedmtllora_A.keys():
            if init_lora_weights is True:
                # initialize A the same way as the default for nn.Linear and B to zero
                # https://github.com/microsoft/LoRA/blob/a0a92e0f26c067cf94747bdbf1ce73793fa44d19/loralib/layers.py#L124
                nn.init.kaiming_uniform_(self.combinedmtllora_A_shared[adapter_name].weight, a=math.sqrt(5))


                nn.init.kaiming_uniform_(self.combinedmtllora_A[adapter_name].weight, a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.combinedmtllora_A_shared[adapter_name].weight, std=1 / self.r[adapter_name])

                nn.init.normal_(self.combinedmtllora_A[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_lora_weights=}")

            nn.init.zeros_(self.combinedmtllora_B_shared[adapter_name].weight)

            nn.init.kaiming_uniform_(self.combinedmtllora_B_w[adapter_name], a=math.sqrt(5))


            for num in range(B_num):
                nn.init.zeros_(self.combinedmtllora_B[adapter_name+"_"+str(num)].weight)

            if self.lora_bias[adapter_name]:
                nn.init.zeros_(self.combinedmtllora_B[adapter_name].bias)
        if adapter_name in self.combinedmtllora_embedding_A.keys():
            # Initialize A to zeros and B the same way as the default for nn.Embedding, see:
            # https://github.com/microsoft/LoRA/blob/4c0333854cb905966f8cc4e9a74068c1e507c7b7/loralib/layers.py#L59-L60
            nn.init.zeros_(self.combinedmtllora_embedding_A[adapter_name])
            nn.init.normal_(self.combinedmtllora_embedding_B[adapter_name])
            if self.lora_bias[adapter_name]:
                # embeddings are not supported at the moment, but still adding this for consistency
                nn.init.zeros_(self.combinedmtllora_embedding_B[adapter_name].bias)

    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        raise NotImplementedError(f"COMBINEDMTL Linear can not be merged")
        

    def unmerge(self) -> None:
        raise NotImplementedError(f"COMBINEDMTL Linear can not be unmerged")
        

    def get_delta_weight(self, adapter) -> torch.Tensor:
        raise NotImplementedError(f"COMBINEDMTL Linear get_delta_weight not implemented")
    ...
    def forward(self, x: torch.Tensor, language_id: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            combinedmtllora_A_keys = self.combinedmtllora_A.keys()
            for active_adapter in self.active_adapters:
                if active_adapter not in combinedmtllora_A_keys:
                    continue
                
                combinedmtllora_A_shared = self.combinedmtllora_A_shared[active_adapter]
                combinedmtllora_B_shared = self.combinedmtllora_B_shared[active_adapter]


                combinedmtllora_A = self.combinedmtllora_A[active_adapter]

                all_B = []
                for num in range(self.B_num[active_adapter]):
                    combinedmtllora_B = self.combinedmtllora_B[active_adapter+"_"+str(num)]
                        # print("combinedmtllora_B")
                        # print(active_adapter+"_"+str(num))
                        # print(combinedmtllora_B.weight)
                    all_B.append(combinedmtllora_B.weight)
                all_B = torch.stack(all_B, dim=0)


                combinedmtllora_B_w = F.softmax(
                    self.combinedmtllora_B_w[active_adapter] / self.B_scale[active_adapter], 
                    dim=-1
                )
                    # print("self.combinedmtllora_B_w[active_adapter]")
                    # print(self.combinedmtllora_B_w[active_adapter])
                    # print("self.combinedmtllora_B_w[active_adapter][lambda_index]")
                    # print(self.combinedmtllora_B_w[active_adapter][lambda_index.item()])

                task_B = torch.einsum('lb,bor->lor', combinedmtllora_B_w, all_B)

                selected_combinedmtllora_B_w = task_B[lambda_index.item()]
                    # print("selected_combinedmtllora_B_w")
                    # print(selected_combinedmtllora_B_w)

                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                x = self._cast_input_dtype(x, combinedmtllora_A.weight.dtype)

                after_shared = combinedmtllora_B_shared(combinedmtllora_A_shared(dropout(x))) * scaling

                after_A = combinedmtllora_A(dropout(x))
                after_B = after_A @ selected_combinedmtllora_B_w.transpose(-2, -1)
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    result = result + after_shared + after_B * scaling
                else:
    ...
    #             combinedmtllora_A = self.combinedmtllora_A[active_adapter]
    #             print("combinedmtllora_A.weight")
    #             print(combinedmtllora_A.weight)
    #             combinedmtllora_B = self.combinedmtllora_B[active_adapter+"_0"]
    #             print("combinedmtllora_B.weight")
    #             print(combinedmtllora_B.weight)
    #             dropout = self.lora_dropout[active_adapter]
    #             scaling = self.scaling[active_adapter]
    ...
    #             if not self.use_dora[active_adapter]:
    #                 result = result + combinedmtllora_B(combinedmtllora_A(dropout(x))) * scaling
    #             else:
    #                 if isinstance(dropout, nn.Identity) or not self.training:
    #                     base_result = result
    #                 else:
    ...
    #                 result = result + self.lora_magnitude_vector[active_adapter](
    #                     x,
    #                     combinedmtllora_A=combinedmtllora_A,
    #                     combinedmtllora_B=combinedmtllora_B,
    #                     scaling=scaling,
    #                     base_layer=self.get_base_layer(),
    #                     base_result=base_result,
    #                 )
...
class COMBINEDMTLEmbedding(nn.Module, COMBINEDMTLLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        COMBINEDMTLLoraLayer.__init__(self, base_layer)
    ...
    def update_layer(
        ...
        self.combinedmtllora_embedding_A[adapter_name] = nn.Parameter(weight_A)
        self.combinedmtllora_embedding_B[adapter_name] = nn.Parameter(weight_B)
        self.lora_bias[adapter_name] = lora_bias
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.combinedmtllora_embedding_A.keys():
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        """
        device = self.combinedmtllora_embedding_B[adapter].device
        dtype = self.combinedmtllora_embedding_A[adapter].dtype
        ...
        weight_A = self.combinedmtllora_embedding_A[adapter]
        weight_B = self.combinedmtllora_embedding_B[adapter]
        ...
        if cast_to_fp32:
            ...
            self.combinedmtllora_embedding_A[adapter] = weight_A.to(dtype)
            self.combinedmtllora_embedding_B[adapter] = weight_B.to(dtype)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            if active_adapter == "__base__":
                continue
            if active_adapter not in self.combinedmtllora_embedding_A.keys():
                continue

            embedding_A = self.combinedmtllora_embedding_A[active_adapter].T
            embedding_B = self.combinedmtllora_embedding_B[active_adapter].T
            scaling = self.scaling[active_adapter]
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            torch_result_dtype = result.dtype
            for active_adapter in self.active_adapters:
                if active_adapter not in self.combinedmtllora_embedding_A:
                    continue
                embedding_A = self.combinedmtllora_embedding_A[active_adapter].T
                embedding_B = self.combinedmtllora_embedding_B[active_adapter].T
                scaling = self.scaling[active_adapter]
...
class _COMBINEDMTLConvNd(nn.Module, COMBINEDMTLLoraLayer):
...
class COMBINEDMTLConv2d(_COMBINEDMTLConvNd):
...
class COMBINEDMTLConv1d(_COMBINEDMTLConvNd):
...
class COMBINEDMTLConv3d(_COMBINEDMTLConvNd):
...
class COMBINEDMTLMultiheadAttention(nn.Module, COMBINEDMTLLoraLayer):
...
def dispatch_combinedmtllora(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        embedding_kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLEmbedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLConv2d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv3d):
        kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLConv3d(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, nn.Conv1d):
        kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLConv1d(target, adapter_name, **kwargs)
        ...
        kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLMultiheadAttention(target, adapter_name, **kwargs)
        ...
        if kwargs["fan_in_fan_out"]:
            ...
            kwargs["fan_in_fan_out"] = combinedmtllora_config.fan_in_fan_out = False
        kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLLinear(target, adapter_name, **kwargs)
    elif isinstance(target_base_layer, Conv1D):
        if not kwargs["fan_in_fan_out"]:
            warnings.warn(
                "fan_in_fan_out is set to False but the target module is `Conv1D`. Setting fan_in_fan_out to True."
            )
            kwargs["fan_in_fan_out"] = combinedmtllora_config.fan_in_fan_out = True
        kwargs.update(combinedmtllora_config.loftq_config)
        new_module = COMBINEDMTLLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)

...
from .layer import COMBINEDMTLConv2d, COMBINEDMTLLoraLayer, dispatch_combinedmtllora
...
class COMBINEDMTLLoraModel(BaseTuner):

    prefix: str = "combinedmtllora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

        print("*********************COMBINEDMTLLoraModel __init__****************************")

    def _check_new_adapter_config(self, config: COMBINEDMTLLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: COMBINEDMTLLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, COMBINEDMTLLoraLayer) and not isinstance(target, AdaLoraLayer):
            raise NotImplementedError(f"isinstance(target, COMBINEDMTLLoraLayer) and not isinstance(target, AdaCOMBINEDMTLLoraLayer).")
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, COMBINEDMTLLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def _create_new_module(mtllora_config, adapter_name, target, **kwargs):
        ...
        dispatchers.extend(
            [
                dispatch_combinedmtllora
            ]
        )

        new_module = None
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, combinedmtllora_config=mtllora_config, **kwargs)
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, COMBINEDMTLLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, COMBINEDMTLLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, COMBINEDMTLLoraLayer) or isinstance(module, AuxiliaryTrainingWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, COMBINEDMTLLoraLayer):
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        conv2d = isinstance(target, COMBINEDMTLConv2d)
        if conv2d:

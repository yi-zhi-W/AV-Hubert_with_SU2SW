...
class HubertFeatureReader(object):
    ...
    def load_feature(self, mix_name, ref_len=None):
        ...
        # print("self.task.cfg.image_crop_size")
        # print(self.task.cfg.image_crop_size)

        audio_fn = audio_fn.split(':')[0]

        wav_data, sample_rate = sf.read(audio_fn)
        assert sample_rate == 16_000 and len(wav_data.shape) == 1
        audio_feats = logfbank(wav_data, samplerate=sample_rate).astype(np.float32)
        audio_feats = stacker(audio_feats, self.stack_order_audio)

        v_len = len(audio_feats)
        image_crop_size = self.task.cfg.image_crop_size
        # 生成形状为 (T, H, W, 1) 的全 0 矩阵
        video_feats = np.zeros((v_len, image_crop_size, image_crop_size, 1), dtype=np.float32)
